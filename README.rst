...
* ``CHANGELOG.rst``
    Historical release timeline from the SourceForge origin through the current
    maintainer era.

* ``CREDITS.rst``
    Historical credits for original developers, maintainers, and contributors.
...
* ``CHANGELOG.rst`` for release history
* ``CREDITS.rst`` for people and stewardship history
...
For project history and acknowledgements, see ``CHANGELOG.rst`` and
``CREDITS.rst``.
